...
이 프로젝트는 논문 *"Topology-Aware Retinal Artery–Vein Classification via Deep Vascular Connectivity Prediction"* 을 기반으로 한 망막 혈관 분석 파이프라인이다. VCP 모델로 혈관을 분할한 뒤, 그래프 추출 → TDA 특징 추출 → 통계 검정 → SVM 분류까지의 전체 흐름을 구현.
...
├── config.py               경로, p-value threshold, plot 설정
├── data_loader.py          Track A + B CSV 병합, 라벨 처리
├── stat_tests.py           ANOVA / Kruskal-Wallis 통계 검정
├── plots.py                Boxplot, Correlation Heatmap
├── selection.py            유의미한 특징 선별
├── main.py                 엔트리포인트
├── statistical_results.csv 통계 검정 결과
├── selected_features.csv   선별된 특징 → 05_SVM 입력
└── plots/                  boxplots.png, correlation_heatmap.png

05_SVM/
├── svm_classifier.py       통합 분류 스크립트 (권장 엔트리포인트)
├── config.py               경로, CV 파라미터, grid search 공간
├── classifier.py           TopologicalSVMClassifier (Pipeline + GridSearchCV)
...
└── results/
    ...
    ├── fold_metrics.csv        fold별 성능 표 (mean ± std)
    └── best_svm_model.pkl      학습 완료 모델
```
...
**입출력**
- 입력: `01_VCP/results/logs/vessel_png/{id}_vessel.png` + `pixelwise_png/{id}_av_pixelwise.png`
- 출력: `02_Graph_Extraction/processed_graphs/{DRIVE,IOSTAR}/{id}_graph.pkl`
...
**배경**: 추출된 12개 특징 중 통계적으로 유의미한 것만 선별하여 분류기에 전달.

#### ML 이전에 통계 검정을 수행하는 이유

DRIVE(20) + IOSTAR(30) = 총 50개의 소규모 샘플에서 12개 특징을 모두 사용하면:

1. **차원의 저주 (Curse of Dimensionality)** — 샘플 수 대비 특징 수가 너무 많으면 SVM이 학습 데이터에 과적합. 경험적으로 $n > 5d$ (샘플 수 > 5 × 특징 수)를 권장하는데, $50 / 12 \approx 4.2$로 이 기준을 충족하지 못함. 통계 검정으로 노이즈 특징을 사전 제거하면 $50 / 2 = 25$로 충분한 비율 확보.
...
4. **해석 가능성** — 의료 분류에서는 "왜 이 진단이 내려졌는가"를 설명할 수 있어야 함. 2개 특징(fractal_dimension, b0_sum_lifespan)이면 결정 경계를 2D로 시각화하여 임상적 해석이 가능.

#### 통계 검정 절차

각 특징에 대해:

1. **정규성 검정**: Shapiro-Wilk test (각 클래스 그룹별)
   - 귀무가설: "데이터가 정규분포를 따른다"
   - 3개 클래스 그룹 모두 $p > 0.05$이면 정규성 가정
2. **그룹 간 차이 검정**:
   - 모든 그룹이 정규: **One-way ANOVA** (F-test)
   - 비정규 그룹 존재: **Kruskal-Wallis H-test** (비모수)
...
| flood_b0_max/sum_lifespan | Kruskal-Wallis | 0.7509 | NO |
| H1 특징 (6개) | — | — | 분산 0 (건너뜀) |

**최종 선별**: `fractal_dimension` + `b0_sum_lifespan` (2개 특징)

#### 시각화

- **Boxplot + Swarmplot**: 클래스별 특징 분포 비교 (개별 데이터포인트 표시)
- **Correlation Heatmap**: Pearson 상관 행렬로 다중공선성 확인

**출력**
- `statistical_results.csv`: 각 특징의 검정 방법, 통계량, p-value
- `selected_features.csv`: 유의미한 특징 + label → 05_SVM 입력
...
**배경**: 선별된 TDA 특징을 기반으로 Normal / Glaucoma / Alzheimer 3-class 분류. 소규모 의료 데이터(N=50)에서 과적합을 방지하고 신뢰할 수 있는 성능 추정을 얻기 위한 여러 장치를 적용.
...
3. `class_weight`는 손실 함수 수준에서 보상하므로 데이터 분포 자체를 왜곡하지 않음
...
`f1_macro`를 scoring 기준으로 사용하는 이유: accuracy는 다수 클래스(Normal)를 올바르게 맞히는 것만으로도 높은 수치를 달성할 수 있으나, macro F1은 각 클래스의 precision-recall을 동일 가중으로 평균하므로 소수 클래스의 성능이 전체 점수에 충분히 반영됨.
...
**다중 클래스 AUROC 계산 방식** — One-vs-Rest (OVR):

1. `probability=True`로 Platt scaling 기반 확률 추정 활성화
2. 각 클래스 $c$에 대해 "클래스 $c$ vs 나머지" 이진 문제로 변환
3. `label_binarize()`로 $y$를 one-hot 인코딩
4. per-class ROC curve → per-class AUC 계산
5. `roc_auc_score(average="macro", multi_class="ovr")`로 macro 평균

Repeated K-Fold에서 ROC 계산: 각 반복에서 테스트 fold의 예측 확률을 누적하고, 반복 횟수로 나누어 평균 확률을 산출한 뒤 최종 ROC를 계산. 이는 단일 fold의 확률보다 안정적인 ROC 곡선을 제공.
...
| 사전 특징 선별 | 12 → 2 features | 차원의 저주 완화 |
...
선별된 특징이 정확히 2개(`fractal_dimension`, `b0_sum_lifespan`)이므로, 학습된 SVM의 결정 경계를 2D 평면에 직접 시각화할 수 있다:

1. 특징 공간의 300×300 그리드에 대해 Pipeline(scaler + SVC)의 `predict()`를 호출
2. 결정 영역을 반투명 색상으로 채우고, 실제 데이터 포인트를 클래스별로 오버레이
3. Pipeline이 내부적으로 scaling을 처리하므로, 그리드는 원래 특징 스케일로 생성

이 시각화는 임상적 해석에 중요: "fractal dimension이 낮고 b0_sum_lifespan이 낮으면 Glaucoma 위험" 등의 직관적 규칙을 확인할 수 있음.

#### 출력

- `results/confusion_matrix.png`: 행 정규화 혼동 행렬 (비율 + 원시 카운트 표시)
- `results/roc_curves.png`: 클래스별 OVR ROC 곡선 + macro AUC
- `results/decision_boundary.png`: 2D 특징 공간에서의 SVM 결정 경계
- `results/fold_metrics.csv`: repeat/fold별 Accuracy/Sensitivity/Specificity/Precision/F1 + mean ± std 행
- `results/best_svm_model.pkl`: Pipeline(scaler + SVC) + LabelEncoder + 특징 이름 (배포용)
...
02_Graph_Extraction/processed_graphs/
...
├── confusion_matrix.png           혼동 행렬
├── roc_curves.png                 OVR ROC 곡선
├── decision_boundary.png          SVM 결정 경계
├── fold_metrics.csv               fold별 성능 표 (mean ± std)
└── best_svm_model.pkl             학습 완료 모델 (Pipeline + Encoder)
```
...
# 2. Graph Extraction — 혈관 마스크 → 그래프 변환
...
# 6. SVM 분류기 — Repeated Stratified K-Fold SVM
cd ../05_SVM
